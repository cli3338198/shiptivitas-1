...
export default class Board extends React.Component {
  ...
  // On mount make refs draggeable
  componentDidMount() {
    // Set draggaeable refs
    this.drake = Dragula([
      this.swimlanes.backlog.current,
      this.swimlanes.inProgress.current,
      this.swimlanes.complete.current,
    ]);
    // Update clients when drag/dropped
    this.drake.on("drop", (el, target, src, sibling) =>
      this.updateClients(el, target, src, sibling)
    );
  }
  // // Remove drake when unmounting
  // componentWillUnmount() {
  //   this.drake.remove();
  // }
  // Update the client when card is dropped
  updateClients(el, target, src, sibling) {
    console.log(sibling);
    // FIGURE THIS OUT!!
    this.drake.cancel(true);

    // Find the current swimlane
    ...
    // Get the client that has to be moved and update status
    const copyOfClientToMove = {
      ...this.state.clients.find((client) => client.id === el.dataset.id),
      status: targetSwimlane,
    };

    // Update the status of client with matching el id
    const copyOfClients = [
    ...
    // Keep order of moved client among sibling clients
    const siblingId = sibling.dataset.id;

    // Update clients in state
    this.setState({
      clients: {
        backlog: newClients.filter(
          (client) => !client.status || client.status === "backlog"
        ),
        inProgress: newClients.filter(
          (client) => client.status && client.status === "in-progress"
        ),
        complete: newClients.filter(
          (client) => client.status && client.status === "complete"
        ),
      },
    });
